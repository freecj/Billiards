	...
	void drawCubeFace() {
		...
		for (int i = 0; i < m_segmentsX; i++) {
			...
			for (int j = 0; j < m_segmentsY; j++) {
				...
				Point temp = iPointToSquare(Point(xcoord, ycoord, 0));
				glTexCoord2f(temp[0], temp[1]);
				glVertex3d(xcoord + width, ycoord, 0);
				temp = iPointToSquare(Point(xcoord + width, ycoord, 0));
				glTexCoord2f(temp[0], temp[1]);
				glVertex3d(xcoord + width, ycoord + height, 0);
				temp = iPointToSquare(Point(xcoord + width, ycoord + height, 0));
				glTexCoord2f(temp[0], temp[1]);
				...
				temp = iPointToSquare(Point(xcoord , ycoord + height, 0));
				glTexCoord2f(temp[0], temp[1]);
				glVertex3d(xcoord, ycoord, 0);
			}
		}
